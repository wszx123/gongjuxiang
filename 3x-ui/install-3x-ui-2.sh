...
prompt_and_setup_ssl() {
    ...
    echo -e "${green}2.${plain} Let's Encrypt for IP Address (6天有效期，自动续订)"
    echo -e "${green}3.${plain} 自定义SSL证书（现有文件路径）"
    echo -e "${green}4.${plain} 跳过SSL（高级设置——仅限反向代理/SSH隧道后端）"
    echo -e "${blue}Note:${plain} 选项1和2需要开放80端口。选项3需要手动配置路径。"
    echo -e "${blue}Note:${plain} 选项4通过普通HTTP为面板提供服务，仅在nginx/Caddy或SSH隧道后安全。"
    ...
    case "$ssl_choice" in
        1)
            ...
            if ssl_cert_issue; then
                ...
                if [[ -n "${cert_domain}" ]]; then
                    ...
                    echo -e "${yellow}SSL设置可能已完成，但域提取失败${plain}"
                    SSL_HOST="${server_ip}"
                fi
            else
        ...
        2)
            ...
            local ipv6_addr=""
            read -rp "您有要包含的IPv6地址吗？（留空跳过）: " ipv6_addr
...
install_x-ui() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        if [ "$service_installed" = false ]; then
            echo -e "${yellow}在tar.gz中找不到服务文件，正在从GitHub下载...${plain}"
            case "${release}" in
                ubuntu | debian | armbian)
        ...
        if [ "$service_installed" = true ]; then
            ...
            echo -e "${red}未能安装x-ui.service文件${plain}"
            exit 1
        fi
    fi
